<?php

if (!defined('BASEPATH'))
    exit('No direct script access allowed');

class Plantel_model extends CI_Model
{

    public $table = 'plantel';
    public $id = 'idplantel';
    public $order = 'DESC';

    function __construct()
    {
        parent::__construct();
    }

    // get all
    function get_all()
    {
        $this->db->order_by($this->id, $this->order);
        return $this->db->get($this->table)->result();
    }

    // get data by id
    function get_by_id($id)
    {
        $this->db->where($this->id, $id);
        return $this->db->get($this->table)->row();
    }
    
    // get total rows
    function total_rows($q = NULL) {
        $this->db->like('idplantel', $q);
	$this->db->or_like('Plantel', $q);
	$this->db->or_like('Ubicacion', $q);
	$this->db->or_like('directivo_iddirectivo', $q);
	$this->db->or_like('estado_idestado', $q);
	$this->db->or_like('del_mun', $q);
	$this->db->or_like('Colonia', $q);
	$this->db->or_like('Calle y Numero', $q);
	$this->db->from($this->table);
        return $this->db->count_all_results();
    }

    // get data with limit and search
    function get_limit_data($limit, $start = 0, $q = NULL) {
        $this->db->order_by($this->id, $this->order);
        $this->db->like('idplantel', $q);
	$this->db->or_like('Plantel', $q);
	$this->db->or_like('Ubicacion', $q);
	$this->db->or_like('directivo_iddirectivo', $q);
	$this->db->or_like('estado_idestado', $q);
	$this->db->or_like('del_mun', $q);
	$this->db->or_like('Colonia', $q);
	$this->db->or_like('Calle y Numero', $q);
	$this->db->limit($limit, $start);
        return $this->db->get($this->table)->result();
    }
    //funcion generadora de select html con clase bootstrap
    //en campo se asigna el campo de la tabla que deseamos filtrar
    function get_dropdown($campo){
        $this->db->select('*');
        foreach ($this->db->get($this->table)->result_array() as $row ) {
            $options[$category['idplantel']] =  $category[$campo];
        }
        return form_dropdown('idplantel', $options, '','class="form-control"');
    }
    //funcion generadora de select html con clase bootstrap
    //campo e id que se desea filtrar pruebas
    function get_dropdown_especific($id,$campo){
        $this->db->select('*');
        $this->db->where($campo,$id);
        foreach ($this->db->get($this->table)->result_array() as $row ) {
            $options[$category['idplantel']] =  $category[$campo];
        }
        return form_dropdown('idplantel', $options, '','class="form-control"');
    }

    // insert data
    function insert($data)
    {
        $this->db->insert($this->table, $data);
    }

    // update data
    function update($id, $data)
    {
        $this->db->where($this->id, $id);
        $this->db->update($this->table, $data);
    }

    // delete data
    function delete($id)
    {
        $this->db->where($this->id, $id);
        $this->db->delete($this->table);
    }

}

/* End of file Plantel_model.php */
/* Location: ./application/models/Plantel_model.php */
/* Please DO NOT modify this information : */
/* Generated by Harviacode Codeigniter CRUD Generator 2016-06-02 12:23:50 */
/* http://harviacode.com */